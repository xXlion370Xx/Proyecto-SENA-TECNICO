...
<head>
	...
	<link href="https://fonts.googleapis.com/css2?family=Josefin+Sans:wght@600&family=Vollkorn&display=swap"
		rel="stylesheet">
...
<body>
	...
	<div class="nav">
		<ul class="nav__ul">
			<ul class="nav__ul">
				...
				<li class="nav__li"><i class=" fas fa-tv"></i><a href="GestionCalibrador.php">Gestion Calibrador</a>
				</li>
				<li class="nav__li"><i class=" fas fa-tv"></i><a href="GestionRecorrido.php">Gestion Recorrido</a></li>
				<li class="nav__li"><i class=" fas fa-tv"></i><a href="GestionVehiRecorrido.php">Gestion
						VehiculoRecorrido</a></li>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-row">
							<input id='modalId' required type="hidden" name="idUsuario" placeholder="idUsuario"
								id="idUsuario"
								value="<?php echo $idUsuario; ?>">
							<input id='modalUsuario' required type="email" name="correoUsuario" id="correoUsuario"
								placeholder="Correo"
								value="<?php echo $correoUsuario; ?>">
							<input id='modalPassword' required type="password" name="passwordUsuario"
								id="passwordUsuario" placeholder="Contraseña"
								value="<?php echo $passwordUsuario; ?>">
							<input type="file" accept="image/*" name="fotoUsuario" id="fotoUsuario"
								value="<?php echo $fotoUsuario; ?>">
							<p style="color: #000"><strong>Tipo usuario</strong></p>
							<select name="tipoUsuario" id="tipoUsuario" required>
								<option name="tipoUsuario"><?php echo $tipoUsuario; ?>
								</option>
							...
							<select name="estadoUsuario" id="estadoUsuario" required>
								<option name="estadoUsuario"> <?php echo $estadoUsuario; ?>
								</option>
					...
					<div class="modal-footer">
						<button type="submit" <?php echo $accionAgregar ?> class="btn
							btn-success" name="Accion" value="Agregar">Agregar</button>
						<button type="submit" <?php echo $acccionModificar ?> class="btn
							btn-warning" name="Accion" value="Modificar">Modificar</button>
						<button type="submit" <?php echo $accionEliminar ?> class="btn
							btn-danger" name="Accion" value="Eliminar">Eliminar</button>
						<button type="submit" <?php echo $accionCancelar ?> class="btn
							btn-primary" name="Accion" value="Cancelar">Cancelar</button>
					</div>
				</div>
			</div>
		</div>
	...
	<button type="button" class="Agregar btn btn-primary BotonAgregar" data-bs-toggle="modal"
	...
	<table id="TablaPanelDeControl">
		<thead>
			...
			<script>
				function guardar(idd) {
					const id = document.getElementById(idd)
					const usuario = document.getElementById(idd + 'u')
					const password = document.getElementById(idd + 'p')
					const tipoUsuario = document.getElementById(idd + 'tu')
					const estado = document.getElementById(idd + 'eu')

					document.getElementById('modalId').setAttribute('value', id.innerText)
					document.getElementById('modalUsuario').setAttribute('value', usuario.innerText)
					document.getElementById('modalPassword').setAttribute('value', password.innerText)
					// localStorage.setItem('id', id.innerText)
					// localStorage.setItem('usuario', usuario.innerText)
					// localStorage.setItem('password', password.innerText)
					// localStorage.setItem('tipoUsuario', tipoUsuario.innerText)
					// localStorage.setItem('estado', estado.innerText)
				}
			</script>
		</thead>
		...
		<tbody>
			...
			<tr>
				<td id="<?php echo $i; ?>"><?php echo $columna['idUsuario'];?>
				</td>
				<td id="<?php echo $i . "u"; ?>">
					<?php echo $columna['correoUsuario'];?>
				</td>
				<td id="<?php echo $i . "p"; ?>">
					<?php echo $columna['passwordUsuario'];?>
				</td>
				<td><img class="img-thumbnail" width="100px"
						src="../../imgUsuarios/<?php echo $columna['fotoUsuario'];?>">
				</td>
				<td id="<?php echo $i ."tu"; ?>">
					<?php echo $columna['tipoUsuario'];?>
				</td>
				<td id="<?php echo $i . "eu";?>">
					<?php echo $columna['estadoUsuario'];?>
				</td>
				<td class="tdSeleccionar">
					<form action="" method="POST">
						<input type="hidden" name="idUsuario"
							value="<?php echo $columna['idUsuario'];?>">
						<input type="hidden" name="correoUsuario"
							value="<?php echo $columna['correoUsuario'];?>">
						<input type="hidden" name="passwordUsuario"
							value="<?php echo $columna['passwordUsuario'];?>">
						<input type="hidden" name="fotoUsuario"
							value="<?php echo $columna['fotoUsuario'];?>">
						<input type="hidden" name="tipoUsuario"
							value="<?php echo $columna['tipoUsuario'];?>">
						<input type="hidden" name="estadoUsuario"
							value="<?php echo $columna['estadoUsuario'];?>">
						<button type="button" name="Accion" class="btn btn-seleccionar"
							onclick="guardar(<?php echo $i?>)"
							data-bs-toggle="modal" data-bs-target="#exampleModal"
							value="Seleccionar">Seleccionar</button>
					</form>
					<?php $i++;?>
				</td>
			</tr>
	...
	<footer>
		...
		<a href="https://www.facebook.com/profile.php?id=100005910945632" target="blank"><img class="ima"
				src="../../img/facebook.png" style="width: 3%"></a>
		<a href="https://twitter.com/Danielillopill3" target="blank"><img class="ima" src="../../img/twitter.png"
				style="width: 3%"></a>
		<a href="https://www.youtube.com/channel/UCzHd5ilTV8jMYYD6k2RXrvw" target="blank"><img class="ima"
				src="../../img/Youtube.png" style="width: 3%"></a>
